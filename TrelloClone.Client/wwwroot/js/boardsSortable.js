...
window.initColumnsSortable = function (containerId, dotNetRef) {
    ...
    container.sortableInstance = new Sortable(container, {
        ...
        onEnd: function (evt) {
            ...
            const orderedIds = Array.from(items).map(item => item.getAttribute('data-column-id'));
    ...
    container.sortableInstance = new Sortable(container, {
        ...
        onEnd: function (evt) {
            ...
            if (fromColumnId === toColumnId) {
                ...
                const orderedIds = Array.from(items).map(item => item.getAttribute('data-task-id'));
                boardDotNetRef.invokeMethodAsync('OnTasksReordered', toColumnId, orderedIds);
            } else {
                boardDotNetRef.invokeMethodAsync('OnTaskMovedBetweenColumns', taskId, fromColumnId, toColumnId, newIndex);
            }
        }
    });
};